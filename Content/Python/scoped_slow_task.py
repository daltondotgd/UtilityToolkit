...
### make_dialog_delayed(delay, can_cancell=False, allow_in_pie=False)
...
def scoped_slow_task_example():
    ...
    with unreal.ScopedSlowTask(len(items), 'Processing items') as task:
        ...
        for item in items:
            ...
            task.enter_progress_frame(1, 'Processing {} ({}/{})'.format(item.get_actor_label(), processed_items, len(items)))
...
def show_cancel_confirmation_dialog():
    unreal.EditorDialog.show_message('Operation canceled', 'Operation canceled by user', unreal.AppMsgType.OK, unreal.AppReturnType.OK)

def ask_for_cancel_confirmation():
    user_confirmation = unreal.EditorDialog.show_message(
        'Are you sure?',
        'Are you 100% positive you want to cancel?',
        unreal.AppMsgType.YES_NO_YES_ALL,
