...
def create_new_material_instance_asset(name, base_material_path):
    ...
    if not unreal.EditorAssetLibrary.does_asset_exist(package_name):
        ...
        material_instance = asset_tools.create_asset(name, path, unreal.MaterialInstanceConstant, unreal.MaterialInstanceConstantFactoryNew())
    else:
        material_instance = unreal.load_asset(package_name)

    base_material = unreal.EditorAssetLibrary.find_asset_data(base_material_path)
    unreal.MaterialEditingLibrary.set_material_instance_parent(material_instance, base_material.get_asset())
    ...
    for asset in selected_assets:
        if isinstance(asset, unreal.Texture):
            ...
            if texture_suffix in suffix_to_parameter_map:
                    ...
                    unreal.MaterialEditingLibrary.set_material_instance_texture_parameter_value(material_instance, parameter_name, asset)
...
def ask_for_base_material_path():
    helper_type = unreal.load_class(None, '/UtilityToolkit/MaterialTools/BP_BaseMaterialSelectDialogHelper.BP_BaseMaterialSelectDialogHelper_C')
    
    base_material_select_dialog_helper = unreal.new_object(helper_type)
    options = unreal.EditorDialogLibraryObjectDetailsViewOptions(show_object_name = True, allow_search = True)
    result = unreal.EditorDialog.show_object_details_view("Select the base material", base_material_select_dialog_helper, options)
